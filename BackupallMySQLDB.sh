...
# backup DB
#mkdir -p /root/dbBackupTemp && mysql -uadmin -p`cat /etc/psa/.psa.shadow` -e "show databases;" | grep -v Database | grep -v "\+\-\-" | awk '{print "mysqldump -u admin -p`cat /etc/psa/.psa.shadow` --skip-add-locks "$0" > /root/dbBackupTemp/"$0"$Fecha.sql"}' > /tmp/script.sh && sh /tmp/script.sh 
